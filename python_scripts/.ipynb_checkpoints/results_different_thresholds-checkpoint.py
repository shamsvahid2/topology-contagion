...
def evaluate_classifier(simple_df, complex_df, thresholds, feature, random_state=42):
    ...
    for theta in thresholds:
        ...
        complex_filtered = complex_df[complex_df['theta'] == theta]
        ...
        print('X_train=',np.sum(X_train.isna()))
        print('y_train=',np.sum(y_train.isna()))
...
def plot_accuracy_together(thresholds,  accuracies_eph, accuracies_corr, accuracies_merge
    ...
    plt.plot(thresholds, accuracies_eph, marker='o', label='EPH')
    plt.fill_between(thresholds, lower_bounds_eph, upper_bounds_eph, alpha=0.2)

    plt.plot(thresholds, accuracies_corr, marker='s', label='corr(order,deg)')
    plt.fill_between(thresholds, lower_bounds_corr, upper_bounds_corr, alpha=0.2)
...
def main():
    ...
    for network_name in ['email_eu_modified', 'conf', 'school']:
        ...
        output_file = f'../result_85stop_10init_nodes-v2/accuracy_vs_threshold_{network_name}_v2.pdf'
