...
def main():
    ...
    stop_ratio = 0.85  # Stop when 90% of nodes are infected
    ...
    for network_name in network_list:
        ...
        for _ in tqdm(range(500)):
            #choosing beta
            beta = random.uniform(0.01,0.05)
            beta = np.round(beta,decimals=4)
            beta_sets.append(beta)


            output_file = f'../beta_regression_result_85stop_10%init_nodes-v3/unweighted/unweighted_simple_{network_name}_beta_{beta}.csv'
        ...
        #saving betas
        np.save(f"../beta_regression_result_85stop_10%init_nodes-v3/unweighted/beta_set_{network_name}.npy",np.array(beta_sets))
