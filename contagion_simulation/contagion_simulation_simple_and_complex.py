...
def perform_contagion_transfers_unweighted_version(
    ...
    infection_type_this_step : dict
        # ADDED earlier: maps node -> code for this step (only for complex mode):
    ...
    infection_type_this_step = {}  # ADDED earlier: per-step record of adoption type for newly infected nodes
    ...
    for node in all_nodes:
        ...
        if mode == 'simple':
            ...
            if infected_neighbors_count >= threshold_complex:
                ...
                infection_type_this_step[node] = 1  # above-threshold adoption
            ...
            # Below-threshold adoption: probabilistic "simple-like" spillover.
                ...
                infection_type_this_step[node] = 2  # below-threshold adoption
    ...
    return (
        ...
        n_simple_infection,
        n_complex_infection,
        infection_type_this_step,  # ADDED earlier
    )
...
def perform_simple_contagion_unweighted(beta_simple, graph, initially_infected, stop_ratio):
    ...
    while len(infected_list) < target_infected:
        ...
        new_infections, infected_list, _, _, _ = perform_contagion_transfers_unweighted_version(
            graph,
            infected_list,
            mode='simple',
...
def perform_complex_contagion_unweighted(threshold, graph, initially_infected, prob_complex, stop_ratio=None):
    ...
    infection_type_per_node : np.ndarray
        For each node (aligned with graph.nodes()):
    ...
    step_of_infection_per_node = [None] * len(graph.nodes)
    # Store infection type per node (0 = seed/never infected, 1 = above, 2 = below).
    infection_type_per_node = [0] * len(graph.nodes)
    ...
    if prob_complex == -1:
        ...
        while True:
            ...
            # Record infection type for nodes newly infected in this step.
            for node_index, infection_code in infection_type_this_step.items():
                node_pos = list(graph.nodes).index(node_index)
                if infection_type_per_node[node_pos] == 0:
                    infection_type_per_node[node_pos] = infection_code
    ...
    return (
        ...
        np.array(infection_type_per_node),
    )
...
# --- Worker: run a single simulation (for multiprocessing) ---
def _run_single_simulation(args):  # ADDED (multiprocessing): helper for one simulation
    """
    Run a single simulation (one sample_it) for either simple or complex contagion.

    Parameters
    ----------
    args : tuple
        (sample_it, contagion_type, params, G)
    ...
    seed : int
        Simulation index (sample_it).
    contagion_param : float or int
        Beta (simple) or theta (complex).
    simple_ratio : float
        simple_infection_ratio for this simulation.
    infection_type : np.ndarray or None
        Per-node infection type (only for complex contagion), or None for simple.
    """
    sample_it, contagion_type, params, G = args  # ADDED (multiprocessing)
    ...
    if contagion_type == 'simple':
        ...
        order = perform_simple_contagion_unweighted(
            beta_simple=beta,
            graph=copy.deepcopy(G),  # preserve original logic: deepcopy graph per run
            initially_infected=copy.deepcopy(list_inf),
            stop_ratio=stop_ratio
        )
        contagion_param = beta
        simple_ratio = 1  # For simple contagion, treat ratio as 1 by convention.
        infection_type = None  # no infection-type tracking for simple mode

    elif contagion_type == 'complex':
        threshold = params['threshold']
        prob_complex = params['prob_complex']
        stop_ratio = params.get('stop_ratio', None)
        order, simple_ratio, infection_type = perform_complex_contagion_unweighted(
            threshold=threshold,
            graph=copy.deepcopy(G),  # preserve original logic: deepcopy graph per run
            initially_infected=copy.deepcopy(list_inf),
            prob_complex=prob_complex,
        ...
        contagion_param = threshold

    else:
        raise ValueError(f"Unknown contagion_type: {contagion_type}")  # ADDED (multiprocessing): safety check

    return order, sample_it, contagion_param, simple_ratio, infection_type  # ADDED (multiprocessing)

# --- Simulation Runner (now parallelized) ---
def run_simulation(network_name, contagion_type, params, iterations, output_file):
    ...
    This version uses multiprocessing to parallelize over `iterations`.
    ...
    infection_type_collection_simulations = []  # Per-simulation infection type matrices for complex contagion

    # Prepare arguments for each simulation to run in parallel.
    args_list = [  # ADDED (multiprocessing)
        (sample_it, contagion_type, params, G)
        for sample_it in range(iterations)
    ]

    # Run simulations in parallel using process pool.
    with ProcessPoolExecutor() as executor:  # ADDED (multiprocessing)
        # executor.map preserves the order of args_list, so results correspond to sample_it order.
        results_iter = executor.map(_run_single_simulation, args_list)  # ADDED (multiprocessing)

        # Wrap with tqdm for a progress bar.
        for order, seed, contagion_param, simple_ratio, infection_type in tqdm(  # ADDED (multiprocessing)
            results_iter,
            total=iterations
        ):
            order_collection_simulations.append(order)
            seed_sim.append(seed)
            contagion_param_sim.append(contagion_param)
            simple_ratio_infection_list.append(simple_ratio)
            if contagion_type == 'complex' and infection_type is not None:
                infection_type_collection_simulations.append(infection_type)
    ...
    # For complex contagion, also save an infection-type CSV.
    if contagion_type == 'complex' and len(infection_type_collection_simulations) > 0:
        infection_type_df = pd.DataFrame(infection_type_collection_simulations)
        infection_type_df['seed'] = seed_sim
        infection_type_df[contagion_type + '_param'] = contagion_param_sim
        infection_type_df['simple_infection_ratio'] = simple_ratio_infection_list

        # Name: original file name with '_infection_type' suffix.
        infection_type_output_file = output_file.replace('.csv', '_infection_type.csv')
        infection_type_df.to_csv(infection_type_output_file, index=False)
        print(f"Infection type details saved to {infection_type_output_file}")
...
if __name__ == "__main__":
    # Required for multiprocessing on Windows/macOS to avoid recursive spawning.
    main()
